...
function TinFront({ card, like }) {
  ...
  return (
    <div className="flip-card__face flip-card__face--front tin-front">
      <span className="tin-hinge" />
      <div className="tin-inner">
        <span className="tin-brand">Deckr Tin Co. · est. {new Date().getFullYear()}</span>
        <AppBadge card={card} className="p-mascot--lg" />
        <h3 className="tin-name" style={nameStyle(card)}>
          {card.projectName || 'Untitled project'}
        </h3>
        <Flavour text={card.description} className="tin-tag" />
        <span className="tin-count">Contains {pieces || 'a few'} pieces</span>
        <div className="tin-spacer" />
        <NetWeight card={card} like={like} />
      </div>
    </div>
  );
